...
import {
  Bot, LogOut, PanelLeft, LayoutDashboard, Cpu, Plug, MessageSquare,
  Shield, Activity, ListTodo, ShoppingCart, CreditCard, FileText, Home, Settings,
} from "lucide-react";
...
const menuItems = [
  ...
  { icon: Cpu, label: "My Agents", path: "/agents" },
  { icon: Plug, label: "Connectors", path: "/connectors" },
  { icon: MessageSquare, label: "Chat", path: "/chat" },
  { icon: Shield, label: "Skill Scanner", path: "/skill-scanner" },
  { icon: Activity, label: "Monitoring", path: "/monitoring" },
  { icon: ListTodo, label: "Job Queue", path: "/jobs" },
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          ...
          <div className="flex flex-col items-center gap-4">
            <h1 className="text-2xl font-semibold tracking-tight text-center">Sign in to OpenClaw</h1>
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Access your AI agent management dashboard. Sign in to continue.
            </p>
          </div>
          <Button onClick={() => { window.location.href = getLoginUrl(); }} size="lg" className="w-full glow-primary">
            Sign in
          </Button>
        </div>
      </div>
        ...
        <Sidebar collapsible="icon" className="border-r-0 bg-sidebar" disableTransition={isResizing}>
            ...
            <div className="flex items-center gap-3 px-2 transition-all w-full">
              ...
              {!isCollapsed && (
                <div className="flex items-center gap-2 min-w-0 cursor-pointer" onClick={() => setLocation("/")}>
                  ...
                  <span className="font-bold text-sm gradient-text truncate">OpenClaw</span>
                </div>
              )}
            </div>
          </SidebarHeader>
          ...
          <SidebarFooter className="p-3">
            <SidebarMenu className="px-0">
              <SidebarMenuItem>
                <SidebarMenuButton onClick={() => setLocation("/")} tooltip="Home" className="h-10">
